                ...
                <div id="photoDetails">
                    ...
                    <div id="optionsPhoto">
                        <form action="{{ route('remove', [$photo->id]) }}" method="POST">
                            ...
                            @if(!Auth::check())
                            <a class="favBtn" href="{{ route ('addFav', [$photo->id]) }}" onclick="return confirm('Debes iniciar sesión para añadir {{$photo->name}} a tu lista de favoritos')"><img src="{{ asset('img/heartBlue.png') }}" class="icon heart"></a>
                            @endif
